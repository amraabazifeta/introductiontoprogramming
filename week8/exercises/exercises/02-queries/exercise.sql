...
SELECT first_name, last_name FROM students WHERE last_name LIKE '%s%';

-- 2.2: Teachers with email ending in @cs50.harvard.edu (school.db)
SELECT first_name, last_name, email FROM teachers WHERE email LIKE '%@cs50.harvard.edu';
...
SELECT code, title, department_id FROM courses WHERE department_id IN (1, 2);

-- 2.6: Students who did NOT enroll in 2018, using NOT IN (school.db)
SELECT first_name, last_name, enrollment_year FROM students WHERE enrollment_year NOT IN (2018);

-- 2.7: Courses sorted by credits (desc), then title (asc) (school.db)
SELECT code, title, credits FROM courses ORDER BY credits DESC, title ASC;
...
SELECT first_name, last_name FROM authors WHERE nationality = 'British' ORDER BY last_name;

-- 2.11: Members with membership_type 'premium' or 'student' (library.db)
SELECT first_name, last_name, membership_type FROM members WHERE membership_type IN ('premium', 'student');

-- 2.12 CHALLENGE: Students with exactly 4-letter first names (school.db)
SELECT first_name, last_name FROM students WHERE first_name LIKE '____';
